...
int64_t memory_file::block_manager::write(const char *buffer, const int64_t offset, const int64_t bufsize)
{
    ...
    // Increased the maximum size to 8Gb instead of 1Gb
    ...
    if(total > 8LL * 1024 * 1024 * 1024)
    {
        throw memfile_exception_parameter("memory file size too large (over 8Gb?!)");
    }
...
//            filename.get_password().c_str());
...
#endif /* !USE_OPENSSL */
    {
        ...
        {
            cli.set_basic_auth(filename.get_username(), filename.get_password());
        }
...
void memory_file::append_ar(const file_info& info, const memory_file& data)
{
    ...
    file_info::int_to_str(buf + 16                 , info.get_mtime(), 12, 10, ' '); // char ar_date[12]
    ...
    file_info::int_to_str(buf + 16 + 12 + 6 + 6 + 8, info.get_size() , 10, 10, ' '); // char ar_size[10]
