...
public:
    ...
    bool load_conffiles();
    void conffiles(wpkgar_manager::conffiles_t& conf_files);
...
void wpkgar_package::conffiles(wpkgar_manager::conffiles_t& conf_files)
{
    conf_files.clear();
    if(load_conffiles())
    {
        for(conffiles_t::iterator it(f_conffiles.begin());
                                  it != f_conffiles.end();
                                  ++it)
        {
            conf_files.push_back(it->first);
        }
    }
}
...
void wpkgar_manager::load_package(const wpkg_filename::uri_filename& filename, bool force_reload, bool skip_data)
{
    ...
    std::shared_ptr<wpkgar_package> package(new wpkgar_package(this, filename, f_control_file_state));
